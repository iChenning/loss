from utils.my_dataset import super_BYOL, MyDataset
...
if __name__ == "__main__":
    ...
    trainset = super_BYOL(txt_path=opt.read_data.train.file_path, transform=train_trans)
    ...
    for i_epoch in range(opt.train.max_epoch):
        ...
        for i_iter, data in enumerate(trainloader):
            ...
            writer.add_scalar('learning rate', optimizer.state_dict()['param_groups'][0]['lr'],
            ...
            if (i_iter + 1) % 10000 == 0:
                ...
                for par in optimizer.param_groups:
                    par['lr'] = lr
            if (i_iter + 1) % 500 == 0:
                torch.save(byol.state_dict(), '%s/net%d.pth' % (log_dir, i_iter))
                # test(byol)
